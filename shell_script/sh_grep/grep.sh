...
INTERVAL=10s # チェック間隔
MAIL_TO=goonet-info@sub.proto-g.co.jp
...
check_process(){
    ...
    do
        ...
        then
            ...
            # TODO:メール送信
            HOST=`hostname`
            sh /usr/goonet/goolib/mail/SendMail.sh "【警告】grep.sh停止" {$MAIL_TO} 1 `sed 's/<!--SERVER-->/{$hostname}/i' conf/diskover.mail`
            exit 1
        fi
        sleep $INTERVAL
    done &
